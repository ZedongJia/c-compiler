...
void printRuntime(Runtime *runtime)
{
    for (int i = 0; i < 61; i++)
        printf("-");
    printf("\n");
    printf("| %10s | %15s | %6s | %5s | %9s |\n", "Lexeme", "Type", "Offset", "Line", "duplicate");
    for (int i = 0; i < 61; i++)
        printf("-");
    printf("\n");
    for (int i = 0; i < runtime->numOfSymbols; i++)
        printf("| %10s | %15s | %6d | %5d | %9d |\n", runtime->symbols[i]->lexeme, runtime->symbols[i]->type, runtime->symbols[i]->offset, runtime->symbols[i]->line, runtime->symbols[i]->duplicate);
}
...
void printEnv()
{
    for (int i = 0; i < env->size; i++)
    {
        printf("\033[35m<--Runtime[ Namespace:%15s | Level:%2d ]-->\033[0m\n", env->ptrs[i]->namespace, env->ptrs[i]->level);
